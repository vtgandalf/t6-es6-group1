...
#define SUCCESS 0
#define DEVICE_NAME "pwm_driver" /* Dev name as it appears in /proc/devices  */
#define DEV_T_LENGTH 32
#define MAX_READ_BUFFER_SIZE 64

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Thimo Ferede");
MODULE_DESCRIPTION("PWM pin device driver");
...
static unsigned int minor_number;

/*
 * Called when a process tries to open a device file, like
 * "cat /dev/pwmX_enable", 
 * "cat /dev/pwmX_freq" or
 * "cat /dev/pwmX_duty"
 */
static int device_open(struct inode *inode, struct file *file)
{
    ...
    // Extract minor number from inode
    // TODO: use file struct instead when accessed by multiple nodes at the same time
...
{
    const int MaxSize =
        length < MAX_READ_BUFFER_SIZE ? length
            : MAX_READ_BUFFER_SIZE; /* make sure we copy at most length bytes */
    char msg[MaxSize];
    int msgLength = 0;
    ...
    msgLength = snprintf(msg, MaxSize, "Minor nr = %d!\n",
                 minor_number);
